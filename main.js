...
if(btnStartSingle) {
    btnStartSingle.addEventListener('click', () => {
        ...
        document.getElementById('lobby').style.display = 'none'; document.getElementById('gameContainer').style.display = 'block'; resizeCanvas();
        if (audioCtx.state === 'suspended') audioCtx.resume(); startGame();
    });
}
...
function startGame() {
    ...
    spawnTiers.forEach(tier => {
        ...
        if (pos) {
            ...
            units.push({ id: Math.random().toString(36).substr(2, 9), type: arr[Math.floor(Math.random() * arr.length)], side: mySide, gridX: pos.col, gridY: pos.row, x: START_X + pos.col * TILE_SIZE + TILE_SIZE/2, y: START_Y + pos.row * TILE_SIZE + TILE_SIZE/2, timer: 0, attackAnimTimer: 0, items: [], hyperTimer: 0 });
        }
    });
...
function requestGacha() {
    if (myGold >= 100) {
        ...
        units.push({ id: Math.random().toString(36).substr(2, 9), type: spawnUnitId, side: mySide, gridX: pos.col, gridY: pos.row, x: START_X + pos.col * TILE_SIZE + TILE_SIZE/2, y: START_Y + pos.row * TILE_SIZE + TILE_SIZE/2, timer: 0, attackAnimTimer: 0, items: [], hyperTimer: 0 });
...
function handleKill(m) {
    ...
    if (m.isBoss && myInventory.length < 5) { let itemKeys = Object.keys(ITEM_DB); myInventory.push(itemKeys[Math.floor(Math.random() * itemKeys.length)]); }
}
    ...
    for (let step = 0; step < gameSpeed; step++) {
        ...
        units.forEach(u => {
            ...
            if (u.timer <= 0) {
                ...
                if (u.items) { u.items.forEach(itemId => { let itemDef = ITEM_DB[itemId]; if (itemDef) { if (itemDef.type === "dmg") buffedDamage = Math.floor(buffedDamage * itemDef.value); if (itemDef.type === "spd") buffedCooldown = Math.floor(buffedCooldown * itemDef.value); if (itemDef.type === "rng") buffedRange += itemDef.value; } }); }
                ...
                if (target) {
                    ...
                    let targets = [target]; if (activeSkill && activeSkill.type === 'passive_multishot') { targets = validMonsters.slice(0, activeSkill.targets).map(v => v.m); }
                    targets.forEach(t => { projectiles.push({ id: Math.random().toString(36).substr(2, 9), x: u.x, y: u.y, targetId: t.id, damage: finalDamage, speed: 10, side: u.side, style: pStyle, color: pColor, splashRadius: pSplash, skill: activeSkill, damageType: uStats.damageType }); });
        ...
        monsters.forEach(m => { 
            // ★ 보스 스킬 체크! (이제 하드코딩이 아닌 castSkill 플래그로 확인)
            if (m.isBoss && m.castSkill) {
                ...
                if (m.skillCooldown <= 0) {
                    ...
                    if (validUnits.length > 0) {
                        let targetUnit = validUnits[Math.floor(Math.random() * validUnits.length)];
                        targetUnit.stunTimer = 90; 
                        spawnVFX('flash', targetUnit.x, targetUnit.y, {color: "rgba(255, 255, 0, 0.4)", radius: 60});
                        addFloatingText("⚡보스 스킬!", targetUnit.x, targetUnit.y - 30, "#ffeb3b");
                    }
                }
            }
            ...
            if (m.stunTimer > 0) { m.stunTimer--; } else {
                let curSpeed = m.speed; if (hasGlobalSlow && !m.ccImmune) curSpeed *= 0.7; 
...
canvas.addEventListener('mousedown', (e) => {
    ...
    if (selectedUnit && draggingUnit === null) {
        ...
        if (pos.x >= 10 && pos.x <= 710 && pos.y >= 850 && pos.y <= 990) {
            if (pos.x >= 590 && pos.x <= 690) { Sound.click(); let basePrice = unitDef.sellPrice || 50; let sellBonusMult = RELIC_ENGINE.getSellBonus(PlayerProfile.relics || {}); let finalSellPrice = Math.floor(basePrice * (1 + sellBonusMult)); myGold += finalSellPrice; units = units.filter(u => u.id !== selectedUnit.id); selectedUnit = null; return; }
            if (unitDef.next) {
                ...
                if (Array.isArray(unitDef.next)) { if (pos.x >= 450 && pos.x <= 580) { Sound.click(); processUpgrade(unitDef.next[1]); selectedUnit = null; return; } if (pos.x >= 310 && pos.x <= 440) { Sound.click(); processUpgrade(unitDef.next[0]); selectedUnit = null; return; } } else { if (pos.x >= 450 && pos.x <= 580) { Sound.click(); processUpgrade(unitDef.next); selectedUnit = null; return; } }
            }
            return; 
        }
        selectedUnit = null; 
...
canvas.addEventListener('mouseup', (e) => {
    const pos = getPointerPos(e);
    if (draggingItem) {
        let targetUnit = null; for (let u of units) { if (Math.hypot(u.x - draggingItem.x, u.y - draggingItem.y) <= 35) { targetUnit = u; break; } }
        if (targetUnit) { if (!targetUnit.items) targetUnit.items = []; if (targetUnit.items.length < 3) { Sound.click(); targetUnit.items.push(myInventory[draggingItem.index]); myInventory.splice(draggingItem.index, 1); } else { addFloatingText("아이템은 최대 3개까지만 가능합니다!", centerX, centerY, "#ff5252"); } }
...
function gameLoop() {
    ...
    units.forEach(u => {
        ...
        if (draggingUnit && draggingUnit.id === u.id) ctx.restore(); if (selectedUnit && selectedUnit.id === u.id) { ctx.strokeStyle = "rgba(255,255,255,0.35)"; ctx.lineWidth = 1; ctx.beginPath(); ctx.arc(u.x, u.y, uDef.range, 0, Math.PI * 2); ctx.stroke(); }
    });
