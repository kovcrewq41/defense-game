...
let countdownTimer = 0; let isSynergyOpen = true;

const STAGE_DB = {
    1: { name: "1. 평화로운 초원", maxWave: 20, hpMultiplier: 1.0 },
    2: { name: "2. 고블린 둥지", maxWave: 20, hpMultiplier: 3.0 },
    3: { name: "3. 붉은 화산", maxWave: 20, hpMultiplier: 8.0 },
    4: { name: "4. 메마른 사막", maxWave: 20, hpMultiplier: 15.0 }
};
let selectedStage = 1; let currentWave = 1;

const GRID_ROWS = 7; const GRID_COLS = 7; const TILE_SIZE = 80; 
const START_X = centerX - (GRID_COLS * TILE_SIZE) / 2; const START_Y = centerY - (GRID_ROWS * TILE_SIZE) / 2;
...
function saveGame() { localStorage.setItem('defenseSaveData', JSON.stringify({ soulStones: PlayerProfile.soulStones, unlockedStage: PlayerProfile.unlockedStage, passives: PlayerProfile.passives, mastery: PlayerProfile.mastery, relics: PlayerProfile.relics })); }
...
class Projectile {
    ...
    draw() { ctx.fillStyle = (this.effect === "slow" || this.effect === "deep_slow") ? "#00e5ff" : (this.effect === "stun" || this.effect === "deep_stun") ? "#cfd8dc" : (this.effect === "crit") ? "#ff1744" : (this.effect === "splash" || this.effect === "deep_splash") ? "#ff9800" : "#ffff00"; ctx.beginPath(); ctx.arc(this.x, this.y, this.radius, 0, Math.PI * 2); ctx.fill(); }
}
...
class Unit {
    ...
    draw() {
        ...
        if (img && img.complete && img.naturalWidth > 0) { 
            ...
            if (this.attackAnimTimer > 10) { frameCol = 0; frameRow = 1; } else if (this.attackAnimTimer > 0) { frameCol = 1; frameRow = 1; } else { frameCol = 0; frameRow = 0; } 
        ...
        if (selectedUnit === this) { let drawRange = this.range; if (this.trait === "ARCHER" && activeSynergies.ARCHER >= 3) drawRange += 80; for (let itemId of this.items) { if (ITEM_DB[itemId].type === "rng") drawRange += ITEM_DB[itemId].value; } ctx.strokeStyle = "rgba(255, 255, 255, 0.4)"; ctx.beginPath(); ctx.arc(this.x, this.y, drawRange, 0, Math.PI * 2); ctx.stroke(); }
    }
...
function calculateSynergies() { activeSynergies = { SWORD: 0, ARCHER: 0, MAGE: 0, SHIELD: 0, ASSASSIN: 0, CANNON: 0, GUNNER: 0 }; const uniqueIds = [...new Set(units.map(u => u.id))]; uniqueIds.forEach(id => { const trait = UNIT_DB[id].trait || id.split('_')[0]; if (trait) activeSynergies[trait]++; }); }
...
const dexButtons = [ new UIButton(centerX - 330, 220, 280, 60, "", "#4e342e", () => { let c = 50 + PlayerProfile.mastery.SWORD * 50; if (PlayerProfile.soulStones >= c) { PlayerProfile.soulStones -= c; PlayerProfile.mastery.SWORD++; updateDexButtonsText(); saveGame(); } }), new UIButton(centerX - 330, 320, 280, 60, "", "#2e7d32", () => { let c = 50 + PlayerProfile.mastery.ARCHER * 50; if (PlayerProfile.soulStones >= c) { PlayerProfile.soulStones -= c; PlayerProfile.mastery.ARCHER++; updateDexButtonsText(); saveGame(); } }), new UIButton(centerX - 330, 420, 280, 60, "", "#1565c0", () => { let c = 50 + PlayerProfile.mastery.MAGE * 50; if (PlayerProfile.soulStones >= c) { PlayerProfile.soulStones -= c; PlayerProfile.mastery.MAGE++; updateDexButtonsText(); saveGame(); } }), new UIButton(centerX - 330, 520, 280, 60, "", "#37474f", () => { let c = 50 + PlayerProfile.mastery.SHIELD * 50; if (PlayerProfile.soulStones >= c) { PlayerProfile.soulStones -= c; PlayerProfile.mastery.SHIELD++; updateDexButtonsText(); saveGame(); } }), new UIButton(centerX + 30, 220, 280, 60, "", "#6a1b9a", () => { let c = 50 + PlayerProfile.mastery.ASSASSIN * 50; if (PlayerProfile.soulStones >= c) { PlayerProfile.soulStones -= c; PlayerProfile.mastery.ASSASSIN++; updateDexButtonsText(); saveGame(); } }), new UIButton(centerX + 30, 320, 280, 60, "", "#d84315", () => { let c = 50 + PlayerProfile.mastery.CANNON * 50; if (PlayerProfile.soulStones >= c) { PlayerProfile.soulStones -= c; PlayerProfile.mastery.CANNON++; updateDexButtonsText(); saveGame(); } }), new UIButton(centerX + 30, 420, 280, 60, "", "#f57f17", () => { let c = 50 + PlayerProfile.mastery.GUNNER * 50; if (PlayerProfile.soulStones >= c) { PlayerProfile.soulStones -= c; PlayerProfile.mastery.GUNNER++; updateDexButtonsText(); saveGame(); } }), new UIButton(centerX - 150, 1100, 300, 80, "뒤로 가기", "#c62828", () => { currentScene = 'MAIN_MENU'; }) ];
